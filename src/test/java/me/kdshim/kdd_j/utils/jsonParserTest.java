...
public class jsonParserTest extends MyMockMvc {
    ...
    public void test2() throws Exception {
        ...
        book.setUrl("http://localhost:8080");
        book.setRecommender("kdshim");
        book.setLink(new Link("http://localhost:8080", CATEGORY.JAVA, "test link", "this is test only link"));
        System.out.println("==============================================");
...
//        String crsTransaction = "CrsTransaction(super=CrsReward(aspId=000137000000000, userId=50000543364, serviceId=000137000257000, rewardDetailTargetId=null,
...
//        amount=50000, orgAmount=50000, originOrgAmount=null, orgAmountCancelYn=null, nrNumber=KMN221223031792766, orgNrNumber=null, trIncenAmount=null,
//        acquirerMerchantType=null, trPoint=0, acquirerId=null, adminSend=null, approvalDatetime=null, reversalStatus=null, trPointExp=0, sendPush=Y, merchantName=null,
//        acquirerMerchantName=null, pointList=null, expAmount=null, expTrAmount=null, expPointAmount=0, expTrIncenAmount=null, posEntryMode=null)";
    ...
    @KDTest
    public void jsonParserTest3_book() throws Exception {
        String bookstr = "Book(id=1, name=test book, url=http://localhost:8080, recommender=kdshim, link=Link(url=http://localhost:8080, category=JAVA, name=test link, memo=this is test only link))";
        StringDto req = new StringDto();
        req.setPlainString(bookstr);

        String json = mvc.perform(MockMvcRequestBuilders.post("/utils/jsonify")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(getObjectMapper().writeValueAsString(req))
                )
                .andExpect(MockMvcResultMatchers.status().is(200))
                .andReturn().getResponse().getContentAsString();

        Book book = getObjectMapper().readValue(json, Book.class);
        System.out.println("==============================================");
        System.out.println(book);
        System.out.println("==============================================");
        assertEquals(book.getName(), "test book");
        assertEquals(book.getUrl(), "http://localhost:8080");
        assertEquals(book.getRecommender(), "kdshim");
        assertEquals(book.getLink().getUrl(), "http://localhost:8080");
        assertEquals(book.getLink().getCategory(), CATEGORY.JAVA);
        assertEquals(book.getLink().getName(), "test link");
        assertEquals(book.getLink().getMemo(), "this is test only link");


    }

    @KDTest
    public void jsonParserTest4_crsTransaction() throws Exception{
        String crs = "CrsTransaction(super=CrsReward(aspId=000137000000000, userId=50000543364, serviceId=000137000257000, rewardDetailTargetId=null, tid=null, hostName=null, failReason=null, status=SUCCESS, rewardCount=1, rewardType=null, p1=null, ms=null, realRewardId=null, yyyymmdd=null), par=Q121B5D2FBFFBD139A5768C573E, acquireType=null, processingCode=211900, mti=0100, merchantId=410226436448501, merchantType=MC, merchantBizNo=null, amount=50000, orgAmount=50000, originOrgAmount=null, orgAmountCancelYn=null, nrNumber=KMN221223031792766, orgNrNumber=null, trIncenAmount=null, acquirerMerchantType=null, trPoint=0, acquirerId=null, adminSend=null, approvalDatetime=null, reversalStatus=null, trPointExp=0, sendPush=Y, merchantName=null, acquirerMerchantName=null, pointList=null, expAmount=null, expTrAmount=null, expPointAmount=0, expTrIncenAmount=null, posEntryMode=null)";

        StringDto req = new StringDto();
        req.setPlainString(crs);
